...
{
    // For each patch, compute dot product between the patch (flattened) and each weight column
...
// Transpose: out = transpose(in) where in is [rows x cols]
...
// Softmax (row-wise) for a matrix [rows x cols]
...
// Encoder: A single transformer encoder layer (with very simple attention)
                 ...
                 const float* h_Wv, const float* h_Wo, float* h_out)
{
    ...
    // Compute scores = Q * Kᵀ   (result is [SEQ_LEN x SEQ_LEN])
    ...
    // Compute attention = scores * V  (result [SEQ_LEN x MODEL_DIM])
    cpuMatMul(scores, V, attention, SEQ_LEN, SEQ_LEN, MODEL_DIM);
    
    // Compute output = attention * Wo  (result [SEQ_LEN x MODEL_DIM])
                                               ...
                                               int imageWidth, int patchSize, int embeddingDim)
{
    ...
    // Shared memory to hold one patch (flattened)
    __shared__ float s_patch[PATCH_VEC_SIZE];
    
    int tid = threadIdx.x;
    // First, load patch pixels into shared memory
    ...
    // The first 'embeddingDim' threads compute the dot product for one output element
    if (tid < embeddingDim) {
         float sum = 0.0f;
         // Unroll the inner loop (over patch elements)
         #pragma unroll
...
__global__ void add_positional_embeddings_tiled(float* embeddings, int num_patches, int embedding_dim)
{
    ...
    extern __shared__ float s_tile[];  // shared memory allocated per block
...
// Kernel to transpose a matrix: out = transpose(in)
...
// Kernel to scale all elements of a matrix by a constant factor
...
// Kernel to apply row-wise softmax to a matrix
...
// Encoder
// The operations: Q = X*Wq, K = X*Wk, V = X*Wv, scores = Q*Kᵀ, scale, softmax,
// attention = scores*V, and finally output = attention*Wo.
void gpu_encoder(const float* d_X, const float* d_Wq, const float* d_Wk,
                 const float* d_Wv, const float* d_Wo, float* d_out)
{
    // Allocate intermediate buffers on device for Q, K, V, K transpose, scores, and attention
    ...
    // Apply row-wise softmax on scores
...
int main()
{
    ...
    // Encoder weights (for the linear projections in the transformer encoder)
    ...
    std::cout << "CPU Full Inference Time (ms): " << cpu_time.count() << std::endl;
    ...
    // Create CUDA events for timing each stage
    ...
    // Patch Embedding
    // Launch one block per patch with blockDim.x = PATCH_VEC_SIZE (1024 threads)
    ...
    // Positional Embedding Addition
    // Using tile size (e.g. 32 threads per block) and allocate shared memory
    int tile_size = 32;
    ...
    // Encoder
    // Using the patched embeddings as input; result is written to d_out
